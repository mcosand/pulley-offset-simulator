...
export const Canvas = observer(({ store }: { store: Store }) => {
  ...
  return (
    <svg width="800" height="600" id="svg-canvas" style={{ border: 'solid 1px #444', maxWidth:'100%', backgroundColor:'#fff' }}
    onPointerMove={move} onPointerUp={up} viewBox="0 0 100 100">
      ...
      <line id="black1" x1={store.anchors.black.x} y1={store.anchors.black.y} x2={store.blackMiddle.x} y2={store.blackMiddle.y} style={{ stroke: '#444', strokeWidth: .4 }} />
      <line id="black2" x1={store.blackMiddle.x} y1={store.blackMiddle.y} x2={store.blackEnd.x} y2={store.blackEnd.y} style={{ stroke: '#444', strokeWidth: .4 }} />
